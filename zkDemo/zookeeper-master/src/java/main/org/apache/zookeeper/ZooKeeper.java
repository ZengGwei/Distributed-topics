...
/**
 * This is the main class of ZooKeeper consumer library. To use a ZooKeeper
 * service, an application must first instantiate an object of ZooKeeper class.
 * All the iterations will be done by calling the methods of ZooKeeper class.
 * The methods of this class are thread-safe unless otherwise noted.
 * <p>
 * Once a connection to a provider is established, a session ID is assigned to the
 * consumer. The consumer will send heart beats to the provider periodically to keep
 * the session valid.
 * <p>
 * The application can call ZooKeeper APIs through a consumer as long as the
 * session ID of the consumer remains valid.
 * <p>
 * If for some reason, the consumer fails to send heart beats to the provider for a
 * prolonged period of time (exceeding the sessionTimeout value, for instance),
 * the provider will expire the session, and the session ID will become invalid.
 * The consumer object will no longer be usable. To make ZooKeeper API calls, the
 * application must create a new consumer object.
 * <p>
 * If the ZooKeeper provider the consumer currently connects to fails or otherwise
 * does not respond, the consumer will automatically try to connect to another
 * provider before its session ID expires. If successful, the application can
 * continue to use the consumer.
 * <p>
 * The ZooKeeper API methods are either synchronous or asynchronous. Synchronous
 * methods blocks until the provider has responded. Asynchronous methods just queue
 ...
 * the ZooKeeper provider. Other successful ZooKeeper API calls can trigger those
 * watches. Once a watch is triggered, an event will be delivered to the consumer
 * which left the watch at the first place. Each watch can be triggered only
 * once. Thus, up to one event will be delivered to a consumer for every watch it
 * leaves.
 * <p>
 * A consumer needs an object of a class implementing Watcher interface for
 * processing the events delivered to the consumer.
 *
 * When a consumer drops the current connection and re-connects to a provider, all the
 * existing watches are considered as being triggered but the undelivered events
 * are lost. To emulate this, the consumer will generate a special event to tell
...
public class ZooKeeper implements AutoCloseable {
     ...
     */
    @Deprecated
    public static final String SECURE_CLIENT = "zookeeper.consumer.secure";
    ...
    /**
     * This function allows a consumer to update the connection string by providing
     ...
     * probabilistic load-balancing algorithm</a> which may cause the consumer to disconnect from
     * its current host with the goal to achieve expected uniform number of connections per provider
     * in the new list. In case the current host to which the consumer is connected is not in the new
     ...
     * from the current host with probability 0.4 and in this case cause the consumer to connect
     * to one of the 2 new hosts, chosen at random.
     * <p>
     * If the connection is dropped, the consumer moves to a special mode "reconfigMode" where he chooses
     * a new provider to connect to using the probabilistic algorithm. After finding a provider,
     * or exhausting all servers in the new list after trying all of them and failing to connect,
     * the consumer moves back to the normal mode of operation where it will pick an arbitrary provider
     * from the connectString and attempt to connect to it. If establishment of
     * the connection fails, another provider in the connect string will be tried
     * (the order is non-deterministic, as we random shuffle the list), until a
     * connection is established. The consumer will continue attempts until the
     * session is explicitly closed (or the session is expired by the provider).

     * @param connectString
     *            comma separated host:port pairs, each corresponding to a zk
     *            provider. e.g. "127.0.0.1:3000,127.0.0.1:3001,127.0.0.1:3002"
     ...
     * the public methods will not be exposed as part of the ZooKeeper consumer
     * API.
     */
    static class ZKWatchManager implements ClientWatchManager {
        ...
        */
        ...
        protected boolean removeWatches(Map<String, Set<Watcher>> pathVsWatcher,
            ...
            // when sees failure from provider.
    ...
    public enum States {
        ...
        /**
         * Returns whether we are connected to a provider (which
         * could possibly be read-only, if this consumer is allowed
    ...
    /**
     * To create a ZooKeeper consumer object, the application needs to pass a
     * connection string containing a comma separated list of host:port pairs,
     * each corresponding to a ZooKeeper provider.
     * <p>
     * Session establishment is asynchronous. This constructor will initiate
     * connection to the provider and return immediately - potentially (usually)
     * before the session is fully established. The watcher argument specifies
     * the watcher that will be notified of any changes in state. This
     * notification can come at any point before or after the constructor call
     * has returned.
     * <p>
     * The instantiated ZooKeeper consumer object will pick an arbitrary provider
     * from the connectString and attempt to connect to it. If establishment of
     * the connection fails, another provider in the connect string will be tried
     * (the order is non-deterministic, as we random shuffle the list), until a
     * connection is established. The consumer will continue attempts until the
     ...
     * connection string. This will run the consumer commands while interpreting
     ...
     *            also be notified for node events
     * @param conf
     *            (added in 3.5.2) passing this conf object gives each consumer the flexibility of
     ...
     *            also be notified for node events
     * @param canBeReadOnly
     *            (added in 3.4) whether the created consumer is allowed to go to
    ...
    public ZooKeeper(String connectString, int sessionTimeout, Watcher watcher,
        ...
        LOG.info("Initiating consumer connection, connectString=" + connectString
     ...
     * consumer connection, these values must be passed as sessionId and
    ...
    /**
     * The session id for this ZooKeeper consumer instance. The value returned is
     * not valid until the consumer connects to a provider and may change after a
     * re-connect.
     *
    ...
    /**
     * The session password for this ZooKeeper consumer instance. The value
     * returned is not valid until the consumer connects to a provider and may
    ...
    /**
     * The negotiated session timeout for this ZooKeeper consumer instance. The
     * value returned is not valid until the consumer connects to a provider and
    ...
    /**
     * Close this consumer object. Once the consumer is closed, its session becomes
     * invalid. All the ephemeral nodes in the ZooKeeper provider associated with
    ...
    public synchronized void close() throws InterruptedException {
        if (!cnxn.getState().isAlive()) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Close called on already closed consumer");
            }
            return;
        }
    ...
    /**
     * Close this consumer object as the {@link #close() } method.
    ...
    /**
     * Prepend the chroot to the consumer path (if present). The expectation of
     * this function is that the consumer path has been validated before this
     * function is called
     * @param clientPath path to the node
     * @return provider view of the path (chroot prepended to consumer path)
     */
     ...
     * <p>
     * If a node is created successfully, the ZooKeeper provider will trigger the
     ...
     * @throws KeeperException if the provider returns a non-zero error code
     ...
     * @throws KeeperException If the provider signals an error with a non-zero
     *   return code.
     * @throws IllegalArgumentException if an invalid path is specified
     */
     ...
     * (1,048,576 bytes). This limit is specified on the provider via
     ...
     * @return the stat of the node of the given path; return null if no such a
     *         node exists.
     * @throws KeeperException If the provider signals an error
     * @throws InterruptedException If the provider transaction is interrupted.
     * @throws IllegalArgumentException if an invalid path is specified
     */
    ...
    /**
     * Return the last committed configuration (as known to the provider to which the consumer is connected)
     ...
     *            provider connection
     * @throws InterruptedException
     *             if the provider transaction is interrupted.
     * @throws KeeperException.NoWatcherException
     *             if no watcher exists that match the specified parameters
     * @throws KeeperException
     *             if the provider signals an error with a non-zero error code.
     * @throws IllegalArgumentException
    ...
    /**
     * String representation of this ZooKeeper consumer. Suitable for things
     * like logging.
     * 
     ...
     * against an ensemble - test consumer may need to know which provider
