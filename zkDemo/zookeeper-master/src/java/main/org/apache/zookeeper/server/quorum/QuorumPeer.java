...
/**
 * This class manages the quorum protocol. There are three states this provider
 * can be in:
 * <ol>
 * <li>Leader election - each provider will elect a leader (proposing itself as a
 * leader initially).</li>
 * <li>Follower - the provider will synchronize with the leader and replicate any
 * transactions.</li>
 * <li>Leader - the provider will process requests and forward them to followers.
...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public static class QuorumServer {
        ...
        /**
         * Performs a DNS lookup for provider address and election address.
         *
        ...
        public void checkAddressDuplicate(QuorumServer s) throws BadArgumentsException {
            ...
            for (InetSocketAddress my: this.myAddrs) {
                ...
                for (InetSocketAddress other: otherAddrs) {
                    if (my.equals(other)) {
                        String error = String.format("%s of provider.%d conflicts %s of provider.%d", my, this.id, other, s.id);
     ...
     * <p>
     * The response has the xid, the id of this provider, the id of the leader,
    ...
    /**
     * Resolves hostname for a given provider ID.
     *
     * This method resolves hostname for a given provider ID in both quorumVerifer
     * and lastSeenQuorumVerifier. If the provider ID matches the local provider ID,
    ...
    private void loadDataBase() {
        try {
            ...
            throw new RuntimeException("Unable to run quorum provider ", ie);
        }
    }
            ...
            while (running) {
                ...
                case LOOKING: //如果是LOOKING，则进入选举流程
                    ...
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        ...
                        // Create read-only provider but don't start it immediately
    ...
    public QuorumVerifier setQuorumVerifier(QuorumVerifier qv, boolean writeToDisk){
        synchronized (QV_LOCK) {
            if ((quorumVerifier != null) && (quorumVerifier.getVersion() >= qv.getVersion())) {
                // this is normal. For example - provider found out about new config through FastLeaderElection gossiping
    ...
    // Leader and learner will control the zookeeper provider and pass it into QuorumPeer.
   ...
   private boolean updateVote(long designatedLeader, long zxid){       
     ...
     * a new provider tries to join the ensemble.
     * 
